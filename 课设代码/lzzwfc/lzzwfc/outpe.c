...
int load_obj_file(char* fname)
{
    ...
    FILE* fobj;
    CoffSym* cfsyms; //全部的符号
    CoffSym* cfsym; //单个符号条目
    char* strs, * csname;
    void* ptr;
    int cur_text_offset;
    int cur_rel_offset;
...
int pe_load_lib_file(char* name)
{
    ...
    if (fp)
    {
        ...
        for (;;)
        {
            ...
            if (0 == *p || ';' == *p)
                continue;
            add_coffsym(sec_dynsymtab, p, array_dll.count, sec_text->idx, CST_FUNC, IMAGE_SYM_CLASS_EXTERNAL); //此处有问题，i>180时程序没有响应了
        }
        ret = 0;
        if (fp)
            fclose(fp);
...
char* get_lib_path()
{
    char path[MAX_PATH];
    char* p;
    GetModuleFileNameA(NULL, path, sizeof(path)); //获取当前执行文件的绝对路径
    p = strrchr(path, '\\'); //指向字符串最后一次出现字符的地方
    *p = '\0';
    strcat(path, "\\lib\\");
    return strdup(path);
}
